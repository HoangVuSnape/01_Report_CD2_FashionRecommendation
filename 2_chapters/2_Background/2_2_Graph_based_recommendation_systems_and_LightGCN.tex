...
\begin{equation}
    E^{(k+1)} = \mathcal{A} E^{(k)},
\end{equation}
where \(E^{(k)} \in \mathbb{R}^{(|\mathcal{U}| + |\mathcal{I}|) \times d}\) is the concatenated user-item embedding matrix at layer \(k\), and \(\mathcal{A} = D^{-1/2} A D^{-1/2}\) is the symmetric normalized adjacency matrix.
