...
Note that the \texttt{text\_only} setting uses the same textual input regardless of the vision encoder (no image features are involved); it is nonetheless reported under both the CLIP and MobileNetV2 encoder groups as two independent training runs. Consequently, the small differences between these paired \texttt{text\_only} runs reflect run-to-run stochasticity (random initialization, edge dropout, and negative sampling) rather than any encoder effect, and they provide an incidental indication of the run-to-run variance acknowledged in the limitations (Section~5.2).
