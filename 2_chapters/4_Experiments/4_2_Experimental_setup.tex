...
\begin{itemize}
    ...
    \item \textbf{Feature Encoders}: We evaluate two vision encoders (MobileNetV2 vs. CLIP) and a textual encoder (TF-IDF) to extract raw features. These features are stored as \texttt{image\_embeddings.npy} and \texttt{text\_embeddings.npy} respectively.
    ...
    \begin{itemize}
        ...
        \item \texttt{tfidf}: Item representation uses textual embeddings only.
...
Note that the \texttt{tfidf} setting is textual-only and therefore independent of the choice of vision encoder; for presentation symmetry it is reported under both the CLIP and MobileNetV2 encoder groups, so the $24$ reported cells correspond to $18$ distinct model--feature combinations.
