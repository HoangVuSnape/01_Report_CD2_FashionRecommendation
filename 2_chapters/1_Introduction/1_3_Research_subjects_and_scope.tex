...
\begin{itemize}
    ...
    \item Multimodal feature extraction techniques for clothing items, specifically visual encoders (MobileNetV2 and CLIP) and textual representation models (BERT, with TF-IDF for lexical similarity).
...
\begin{itemize}
    ...
    \item \textbf{Feature Scope}: Visual features are extracted using pre-trained, frozen MobileNetV2 and CLIP models, and textual features are extracted using a pre-trained BERT model (with TF-IDF used for CombiGCN's lexical similarity graph), without fine-tuning the raw encoders.
