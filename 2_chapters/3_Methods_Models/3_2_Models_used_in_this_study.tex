...
\begin{enumerate}
    \item \textbf{Precomputed Similarity Graph}: Prior to training, an item-item similarity matrix $\mathbf{W} \in \mathbb{R}^{|\mathcal{I}| \times |\mathcal{I}|}$ is constructed at the data layer. Depending on the configuration of \texttt{sim\_type} (\texttt{img\_only}, \texttt{text\_only}, or \texttt{multimodal}), cosine similarities are computed between the item features (for CombiGCN, the \texttt{text\_only} graph uses TF-IDF lexical similarity). To eliminate noisy connections and enforce sparsity, a threshold filter is applied such that $W_{ij} = 0$ if the cosine similarity is below $0.5$. The matrix is then symmetrically normalized as:
    \begin{equation}
